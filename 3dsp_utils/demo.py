...
def bbox_tracklet(args):
    ...
    # Create a minimal args_track object with default values
    class TrackArgs:
        def __init__(self):
            self.ablation = False
    ...
    file = args.target_clip.split("/")[-1]
    sn_demo(args.target_clip, args.yolov8_param, args.root+"/tracking_result", file, args_track, args)
...
def select_tracklets(args):
    ...
    # read the tracklet - handle both full path and filename cases
    if hasattr(args, 'video_filename') and args.video_filename:
        # Use the clean filename provided by the server
        video_name = args.video_filename.split(".")[0]
    else:
        # Extract from target_clip path
        video_name = args.target_clip.split("/")[-1].split(".")[0]
    
    tracklet_df=pd.read_csv(args.root+"/tracking_result/"+video_name+".txt")
        ...
        if len(traklet) < 20:
            for k in range(1,21):
                if k not in traklet['frame'].values:
                    ...
                    tracklet_df = pd.concat([tracklet_df, nearest_bbox], ignore_index=True)
    image_dict = {}
    for traklet_id in tracklet_df['id'].unique():
    ...
    #read video
    cap = cv2.VideoCapture(args.target_clip)
    fps = 25
    image_width = 96
    image_height = 96
    for j in range(fps):
        ...
        if ret:
            if j<=14 and j>=9:
                ...
                for index, row in frame_bbox.iterrows():
                    ...
                    # save the bbox
                    if args.save_image:
                        out_path = os.path.join(args.root+"/bbox_image", video_name+"_"+str(int(row["id"])))
...
def gen_3d_pose(args):
    ...
    img2video_demo(args.root+"/shooter_image", args.root,args.target_clip.split("/")[-1].split(".")[0])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t","--target_clip", type=str, default=os.getcwd()+"/example/test_00003.mp4", help="target clip to be considered")
    args = parser.parse_args()
    args.root = os.path.join(args.root, args.target_clip.split("/")[-1].split(".")[0])
